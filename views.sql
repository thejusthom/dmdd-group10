    CREATE OR REPLACE VIEW doctor_information_view AS
    ...
    CREATE OR REPLACE VIEW patient_information_view AS
    ...
    CREATE OR REPLACE VIEW appointment_information_view AS
    SELECT appointment.appointment_id, appointment.appointment_date_time AS appointment_date,
    ...
    JOIN patient ON appointment.patient_patient_id = patient.patient_id
    JOIN person ON patient.person_person_id = person.person_id
    WHERE trunc(appointment.appointment_date_time) = TRUNC(SYSDATE);


    CREATE OR REPLACE VIEW top_blood_donors_view AS
